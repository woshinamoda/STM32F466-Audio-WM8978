...
u8 TP_Init(void)
{
	...
	if(Adujust)
	{
		...
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=-0.080957;
			tp_dev.xoff=145;
			tp_dev.yfac=-0.090430;
			tp_dev.yoff=167;
		}else if(USE_HORIZONTAL==1)
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=0.081031;
			tp_dev.xoff=-23;
			tp_dev.yfac=0.089753;
			tp_dev.yoff=-7;
		}else if(USE_HORIZONTAL==2)
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=-0.088106;
			tp_dev.xoff=168;
			tp_dev.yfac=0.078712;
			tp_dev.yoff=-12;
		}else
		{//***因触摸屏批次不同等原因，默认的校准参数值可能会引起触摸识别不准，建议校准后再使用，不建议使用固定的默认校准参数
			tp_dev.xfac=0.092237;
			tp_dev.xoff=-12;
			tp_dev.yfac=-0.076923;
			tp_dev.yoff=140;
		}
		return 0;
	}else
